...
import { getGifConverterService } from "@/lib/services/gif-converter-service"
...
export default function UploadSection({ files, setFiles, onContinue, disabled, setActiveTab }: UploadSectionProps) {
  ...
  const [maxFileSizeMB, setMaxFileSizeMB] = useState<number>(50) // Default value
  ...
  useEffect(() => {
    ...
    converterService
      ...
      .catch((error) => {
        ...
        setMaxFiles(15) // Use a sensible default
      })
    ...
    fetch(`${converterService.getApiBaseUrl()}/api/settings/max-file-size`)
      ...
      .catch((error) => {
        ...
        // Keep default value if there's an error
        setMaxFileSizeMB(50) // Use a sensible default of 50MB
      })
  }, [converterService])
  ...
  const processFiles = (newFiles: File[]) => {
    ...
    const gifFiles = newFiles.filter((file) => file.name.toLowerCase().endsWith(".gif") || file.type === "image/gif")

    if (gifFiles.length === 0) {
      setError("Please select GIF files only.")
      return
    }

    // Check file size (using maxFileSizeMB from settings)
    const maxSizeBytes = maxFileSizeMB * 1024 * 1024
    const validFiles = gifFiles.filter((file) => file.size <= maxSizeBytes)

    if (validFiles.length < gifFiles.length) {
  ...
  return (
    <div>
      <h3 className="text-xl font-semibold mb-4">Upload GIF Files</h3>
      ...
      >
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          <div>
            <p className="text-lg font-medium">Drag & drop your GIF files here</p>
