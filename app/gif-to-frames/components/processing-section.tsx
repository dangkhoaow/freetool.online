...
import type { ConversionJob } from "@/lib/services/gif-converter-service"
...
export default function ProcessingSection({ files, settings, progress, error, job }: ProcessingSectionProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center text-gray-500">{calculateProgress()}% complete</div>
      ...
      {job && job.files && job.files.length === 0 && (
        <div className="bg-blue-50 p-4 rounded-lg max-w-md mx-auto">
          ...
          <p className="text-sm text-blue-800">Uploading {files.length} GIFs to server...</p>
