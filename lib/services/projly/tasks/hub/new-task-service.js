...
export class TasksHubService {
    ...
    try {
      ...
      if (filters.assignedTo && filters.assignedTo !== 'all') {
        ...
        if (Array.isArray(filters.assignedTo)) {
          ...
          const cleanAssignees = filters.assignedTo.filter(id => id && id.trim() !== '');
          if (cleanAssignees.length > 0) {
            queryParams.assignedTo = cleanAssignees.join(',');
            console.log(`${LOG_PREFIX} DEBUG: Multi-assignee query param:`, cleanAssignees.join(','));
          } else {
            console.log(`${LOG_PREFIX} DEBUG: Empty assignee array, no filter applied`);
          }
        } else {
          queryParams.assignedTo = filters.assignedTo;
          console.log(`${LOG_PREFIX} DEBUG: Single assignee query param:`, filters.assignedTo);
        }
      } else {
        console.log(`${LOG_PREFIX} DEBUG: No assignedTo filter:`, filters.assignedTo);
      }
  ...
  async getLabels() {
    try {
      ...
      console.log(`${LOG_PREFIX} Successfully fetched ${response.labels?.length || 0} labels`);
